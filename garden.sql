...
CREATE TABLE INSTRUCTIONS (actionType TEXT NOT NULL, stepNum INTEGER NOT NULL, plantName TEXT NOT NULL REFERENCES PLANT (plantName) ON DELETE CASCADE ON UPDATE CASCADE, materialID INTEGER NOT NULL REFERENCES MATERIALS (materialID) ON DELETE CASCADE ON UPDATE CASCADE, instructionText TEXT, PRIMARY KEY (actionType, stepNum, plantName, materialID));
...
CREATE TABLE MATERIALS (materialID INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, name TEXT, price DECIMAL);
...
CREATE TABLE PLANT (plantName TEXT UNIQUE NOT NULL, depth REAL, growTemp TEXT, germTemp TEXT, illnessName TEXT REFERENCES HEALTH (illnessName) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY (plantName));
...
CREATE TABLE PROJECT (projectID INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, gardenName TEXT);
...
CREATE TABLE PROJECT_PLANT (projectID INTEGER REFERENCES PROJECT (projectID) ON DELETE CASCADE ON UPDATE CASCADE UNIQUE NOT NULL, plantName TEXT REFERENCES PLANT (plantName) ON DELETE CASCADE ON UPDATE CASCADE NOT NULL UNIQUE, PRIMARY KEY (projectID, plantName));
...
CREATE TABLE PROJECT_STATUS (userID INTEGER REFERENCES USER (userID) ON DELETE CASCADE ON UPDATE CASCADE UNIQUE NOT NULL, projectID INTEGER REFERENCES PROJECT (projectID) ON DELETE CASCADE ON UPDATE CASCADE UNIQUE NOT NULL, status, PRIMARY KEY (userID, projectID));
...
CREATE TABLE USER (userID INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, name TEXT, encryptedPassword VARCHAR (50));
